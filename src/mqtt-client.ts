...
import { IClientOptions, IClientPublishOptions, MqttClient as MQTTClientType } from "mqtt"

const MQTT_TIMEOUT_SECONDS = 3
const MQTT_TIMEOUT_MS = MQTT_TIMEOUT_SECONDS * 1000

const KEEP_ALIVE_SECONDS = 5
const RECONNECT_INTERVAL_MS = 1 * 1000 // 1 second
...
class MqttClient {
  ...
  connect(url: string, connectCallback?: () => void): void {
    ...
    this.mqttClient.on("close", () => {
      ...
      this.mqttClient?.end(true, {}, () => {
        ...
        setTimeout(() => this.connect(url, connectCallback), RECONNECT_INTERVAL_MS)
      })
    })
  ...
  subscribe(topic: string, regex: RegExp, cb: Callback): void {
    ...
    this.mqttClient?.subscribe(topic, (err: Error) => {
      if (err) {
        this.subscriptions = this.subscriptions.filter(sub => sub.pattern !== regex || sub.callback !== cb)
  ...
  unsubscribe(topic: string, regex: RegExp, cb: Callback): void {
    this.subscriptions = this.subscriptions.filter(sub => sub.pattern !== regex || sub.callback !== cb)
    const sameTopicSubscriptions = this.subscriptions.filter(sub => String(sub.pattern) === String(regex))
